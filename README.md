...
## All source code and retrieved data for __insert name of research paper here__.

In plain terms, this project involves testing AI's prediction of suspect information when given a crime scenario. These scenarios are generated by pulling court case opinions from Court Listener through its official API (https://wiki.free.law/c/courtlistener/help/api/rest/v4/rest-api-v47). As opinions may widely range in length, they are then summarized via Claude.

## Court case opinion retrieval methodology:
...
3. The script 'retrieveCases.py' was created to call the Court Listener API using their 'search' url, and download all recorded cases present between August 10, 2025 and August 9th, 2026—an arbitrary time frame that is a year in length. The generated output file is called 'all_cases.csv'. This and subsequent csv files were moved into a 'data' folder.
...
5. The script 'cleanCases.py' was then run to lightly clean the data. The output file is called 'cleaned_cases.csv'.
